...
Vamos tomar como primeiro exemplo a fórmula do teorema de Pitágoras. Vamos escrever no prompt a seguinte expressão: \textrm{c\^\ 2+b\^\ 2=a\^\ 2}

	\begin{figure}[htbp!]
    \centering
    \caption{Entrando um expressão algébrica.}
        \label{fig:pitagoras}
    %\begin{Center}
    \includegraphics[scale=0.6]{pitagoras.jpg}
	%\end{Center}
\end{figure}

\noindent Essa expressão é o input do processo. O Maxima processará essa entrada após acionada a combinação de tecla \emph{Shift+Enter} e, em seguida, apresentará uma saída na área do \emph{prompt}, figura \ref{fig:pitagoras-2}.

	\begin{figure}[htbp!]
    \centering
    \caption{Saída apresentada após a execução de um comando por meio de uma expressão algébrica.}
        \label{fig:pitagoras-2}
    %\begin{Center}
    \includegraphics[scale=0.6]{pitagoras-2.jpg}
	%\end{Center}
\end{figure}
\noindent Observe que, a resposta disponibilizada recebe identificadores, tais que
\begin{itemize}
\item \begin{minipage}[t]{4.000000em}\color{red}\bfseries (\% i1)	
\end{minipage}, indica que a expressão na linha é o \emph{input}, e
%%%%
\item \begin{minipage}[t]{4.000000em}\color{green}\bfseries (\% o1)	
\end{minipage}, indica que a expressao na linha é a saída (\emph{output}).
\end{itemize}

No Maxima é possível nomear um expressão de forma que possa ser reutilizada. Por exemplo, escrevendo no prompt a seguinte expressão: \textrm{pitg:c\^\ 2+b\^\ 2=a\^\ 2} e observar o resultado disso na figura \ref{fig:pitagoras-3}.
	\begin{figure}[htbp!]
    \centering
    \caption{Nomeando expressões.}
        \label{fig:pitagoras-3}
    %\begin{Center}
    \includegraphics[scale=0.6]{pitagoras-3.jpg}
	%\end{Center}
\end{figure}

\noindent Ainda, na mesma sessão, aprofundemos o uso de expressões, com a seguinte expressão nomeada \textrm{sol1:solve(pitg,a)}. O termo \emph{solve} corresponde a uma fórmula (função) interna do Maxima, os resultados aparecem na figura \ref{pitagoras-4}. Ao aplicar um termo \emph{solve}, o Maxima resolve, no caso, a expressão \emph{pitg} para a literal \emph{a}, retornando duas soluções, escritas como um vetor linha de dois elementos. 

Explorando mais além vamos ver a praticidade do uso de nomes para as expressões.
	\begin{figure}[htbp!]
    \centering
    \caption{Usando expressões.}
        \label{fig:pitagoras-4}
    %\begin{Center}
    \includegraphics[scale=0.6]{pitagoras-4.jpg}
	%\end{Center}
\end{figure}
%%%
...
\begin{itemize}
	...
	\item Pressing Ctrl+Space or Ctrl+Tab starts an autocomplete function that can not only complete all functions that are integrated into the Maxima core and their parameters: It also knows about parameters from currently loaded packages and from functions that are defined in the current file.
\end{itemize}
