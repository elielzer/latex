...
\begin{equation*}
	{\partial^2 \xi \over \partial t^2} = {T\over m}\, { \partial^2 \xi \over \partial x^2  }
\end{equation*}
\begin{figure}[H]
	\centering
...
Uma vez que se estabeleça um sistema de referência então um observador, sob a perspectiva de um experimento ou hipótese, descreverá suas observações por meio de uma terminologia, a qual traduz, de certa forma, as propriedades do movimento ondulatório.\\
\begin{itemize}
	\item Há a \emph{perturbação}, que é a ação inicial, intencional ou não, que leva a efeito a produção do que vem a ser uma onda. O local onde houve a perturbação marca origem ou a \emph{fonte}. Então uma onda diz-se que é \emph{produzida}.
	\item Uma vez que a ação perturbatória resultou em uma onda sobre o meio então essa onda é chamada de um \emph{pulso}. O pulso é \emph{enviado} e possui perspectiva de direção, tempo e espaço. Portanto o pulso viaja ou caminha.
	...
	\item Todos os pontos do meio sujeito a uma mesma onda possuem o mesmo \emph{estado de movimento}. Tais pontos constituem uma \emph{frente de onda}
...
%
	\item As partículas do meio afetadas pela perturbação da onda efetuam apenas um movimento de oscilação em torno de sua posição original ou de equilíbrio, ou seja, não há um deslocamento resultante para elas. Portanto, a \emph{amplitude} se configura pela medida do deslocamento de um elemento do meio submetido ao movimento perturbatório a partir da posição de equilíbrio.
%
	\item É de grande interesse para o estudo da Ondulatória o perfil \emph{senoidal}.
%	
	\item O estado da onda, que pode ser estacionário, ou em propagação.
	
	\item O tipo de onda quanto ao seu princípio físico governante, que devem ser: \emph{ondas mecânicas},  \emph{eletromagnéticas} e \emph{materiais}.
...
\begin{equation*}
	y=   a\,\cosh\qty({x\over a})\,\qty[ 2\, \sin \qty(k\,x)\,\cos\qty(\omega\,t)]
\end{equation*}
É bom estar atento a que o parâmetro \emph{a} nem sempre corresponderá a um referencial de altura em uma situação real. A parte trigonométrica já está expressa tendo sido levado em consideração o efeito ondulatório sobre o fio preso nas suas duas extremidades, que induziria a um estado estacionário, assumindo-se o modelo harmônico para governar o processo.\\
