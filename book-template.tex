...
Ainda de acordo com esse site o Maxima é uma versão evoluída de software especialista Macsyma. Os sistemas especialistas foram criados com a finalidade de reproduzir o raciocínio ou expertise de um  profissional de alguma área específica do conhecimento.
...
É importante notar que existem características diferentes em tais situações. Em algumas simplesmente há oscilações ou vibrações e, em outros casos, além de oscilar ou vibrar, essa condição é posteriormente transmitida para outros locais do ambiente longe de sua origem. Neste segundo caso, diz-se que há uma propagação do fenômeno. De tal forma que, \emph{oscilação}, \emph{onda}, \emph{propagação} são termos correlacionados e condicionam a natureza do movimento ondulatório de interesse da física.
...
O termo que aparece nessa última expressão, na forma \(  \nicefrac{\lambda}{v} \) é denominado \emph{período de oscilação}, \( P \), associado a cada cada ponto \( x \) sujeito ao efeito da onda ("os vaivéns"), é expresso em unidades de tempo. Consequentemente define-se a \emph{frequência angular} da onda, \( \omega \), em radianos por unidade de tempo
\begin{equation}\label{freqAngular}
	\omega\, =\, \frac{2\, \pi}{P}\text{.}
\end{equation}
Portanto, para um onda harmônica, movendo-se (propagando-se) no sentido positivo do eixo X escreve-se, suas função de onda na forma:
\begin{equation}\label{harmonica}
	\xi\qty(x,t) = \xi_0 \sin\qty(k\,x - \omega\,t)\text{.}
\end{equation}
Uma vez que ciclo completo de oscilação se associa o valor de \( 2\, \pi\, \mathrm{rad}\), podemos definir uma grandeza $\nu$ por meio da expressão,
...
\newpage 
O campo elétrico e o campo magnético são grandezas acopladas formando o chamado de \emph{campo eletromagnético}. Isso é pode ser evidenciado mediante a equação vetorial $\vec{B} \,=\, \mu_0 \, \varepsilon_0 \, \vec{v} \times \vec{E} $ associada a uma carga puntual em movimento (não acelerada).

Ao aplicar as equações de Maxwell a campos elétricos e magnéticos cruzados, ou seja, o campo eletromagnético, chega-se a uma expressão na forma diferencial:
\begin{equation*}
	\label{frequencia}
	{\partial^2 {E}\over \partial t^2 }  \, =\, {1 \over { \mu_0 \, \varepsilon_0} } \, { \partial^2 {E}\over \partial x^2  }  \text{,}
\end{equation*}
e tal equação indica que o campo eletromagnético propaga-se, abstraindo-se das particularidades, como uma onda plana no espaço, isso porque tal expressão é compatível com a equação  diferencial da onda (o mesmo valerá para o vetor $\vec{B}$).
...
\end{align}
as quais podem ser representadas graficamente.\\
\begin{minipage}[t]{\textwidth}\color{blue}
	\centering
	\noindent
	\texttt{CODE CELL MAXIMA}
...
\begin{minipage}[t]{\textwidth}\color{blue}
	...
	wxdraw3d(\ xlabel=\ensuremath{``x''},ylabel=``y'',\ zlabel=``z'',\ grid=true,\ \\
...
Vamos considerar agora uma superfície, tal como uma membrana retangular com suas bordas fixadas, uma vez que essa superfície seja perturbada, são produzidas ondas que se propagam em todas as direções e refletidas nas bordas, acarretando uma interferência. A equação que governa essa situação na condição estacionária, correspondendo a um sistema de quatro ondas será:
\begin{equation*}
	\xi \,=\,-4\, \xi_0\, \sin k_1\,x \, \sin k_2\,y\, \sin \omega\,t \text{,}
\end{equation*}
onde \(k_1\,=\, {n_1\,\pi \over a }  \), \(k_2\,=\, {n_2\,\pi \over b }  \), \(n_1\), \(n_2\), são números inteiros, \(a\) e \(b\) correspondem às dimensões da membrana. Supondo-se ondas produzidas com uma única frequência esta será dada por \(\omega\,=\, \sqrt{k_1^2+k_2^2}\,c\).
...
\end{minipage}
\begin{minipage}[t]{\textwidth}\color{blue}
	A:\ [a:\ 2,\ b:\ 3,\ n\_1:\ 2,\ n\_2:\ 2,\ c:1,\ \ensuremath{\xi}\_0:\ 10\ ]\$\ k\_1:\ A[3]*\%pi/A[1]\$\ \ k\_2:\ A[4]*\%pi/A[2]\$\\
	\ensuremath{\omega}:\ sqrt(k\_1\^\ 2+\ k\_2\^\ 2)*A[5]\$\ t:[0,4,8]\$\ \ensuremath{\xi}(x,y,t):=-4*A[3]*\ sin(k\_1*x)*sin(k\_2*y)*sin(\ensuremath{\omega}*t)\$\\
	wxdraw3d(font\ \ =\ "Arial",\ font\_size\ =\ 20,\ title\ ="Ondas\ estacionárias\ em\ duas\ dimensões",\\
	\ \ \ \ key\ =\ concat("t:"\ ,t[1]),\ color\ =\ gray,\ explicit(\ensuremath{\xi}(x,y,t[1]),x,0,a,y,0,b),\\
	\ \ \ \ key\ =\ concat("t:",\ t[2]),\ color\ =\ red,\ \ explicit(\ensuremath{\xi}(x,y,t[2]),x,a/3,a,y,0,b),\\
	\ \ \ \ key\ =\ concat("t:",\ t[3]),\ color\ =\ blue,\ explicit(\ensuremath{\xi}(x,y,t[3]),x,0,a,y,0,b),\ axis\_3d\ =\ false,\ \\
	\ \ \ \ contour\ =\ none,\ surface\_hide\ \ =\ true,\ ip\_grid=[1,1])\$
\end{minipage}
\noindent%
\begin{figure}[h]
	\centering
	\includegraphics[width=0.85\linewidth]{estacionárias_3D}
	\label{fig:estacionarias3d}
\end{figure}

\chapter{APLICAÇÕES ONDULATÓRIAS}
De modo mais geral, o estudo do movimento ondulatório é feito do ponto de vista da dinâmica do processo, ou seja, é assumindo que haja uma perturbação em um ponto e ela é propagada até outro ponto, o que conduz tal processo a ser descrito por uma equação dinâmica, ou diferencial.
...
\end{figure}
\section{Radiação de um Dipolo Elétrico Oscilante}
Um dipolo elétrico é um dispositivo  que representa o efeito físico de duas cargas elétricas simétricas e separadas a uma pequena distância. Polaridade é a propriedade que determina a intensidade de um dipolo e é representada pelo vetor momento de dipolo. 
Uma forma de gerar ondas eletromagnéticas é fazendo oscilar o valor da polarização. O valor do campo elétrico irradiado é determinado pelas equações de Maxwell. Uma possível solução leva à expressão:
\begin{align*}
	\xi\,=\, {\Pi_0\, \sin\theta \over 4\,\pi\,\epsilon_0\,r}\,\qty(\omega\over c)^2 \, \sin\qty(k\,r-\omega\,t)	\text{,}
\end{align*}
onde \(\Pi_0\) corresponde ao valor de pico do momento de dipolo elétrico e \(\theta\) é a coordenada angular em relação ao eixo do dipolo.
\newline

\setcounter{exemplos}{1}
{ {\scshape Exemplo \thesection.\theexemplos:}} Para manter um dipolo elétrico irradiando devemos fornecer-lhe energia. Tal energia é transportada pela onda irradiada que ter-lhe-á associada a grandeza \emph{intensidade de onda} à medida em que é estabelecida um distribuição espacial de sua potência. Represente graficamente a intensidade da onda irradiada em coordenadas polares. Considere a intensidade a partir da relação  \(I\qty(\theta)\,=\, c\, \mathtt{E}_{\mathrm{med}}\), onde corresponde à densidade média de energia irradiada por uma onda.

\noindent
Solução.\\
A densidade total de energia de uma onda eletromagnética é dada por;
\begin{align*}
   \mathtt{E}\,=\, c\, \xi^2   	\text{.}
\end{align*}
Substituindo pela expressão sugerida para \(\xi\)
\begin{align*}
	\mathtt{E} \, =\, c\, \qty[{\Pi_0\, \sin\theta \over 4\,\pi\,\epsilon_0\,r}\,\qty(\omega\over c)^2 \, \sin\qty(k\,r-\omega\,t)]^2	\text{,}
\end{align*}
e aplicando o teorema do valor médio então obtêm-se
\begin{align*}
	 \mathtt{E}_{\mathrm{med}} \,= \,  {\Pi_0^2\,\omega^4 \over 32\,\pi^2\,c^4\,\epsilon_0\,r^2}\, \sin^2\theta	\text{,}
\end{align*}
para obter a expressão 
\begin{align*}
	I\qty(\theta)\, = \,   {\Pi_0^2\,\omega^4 \over 32\,\pi^2\,c^3\,\epsilon_0\,r^2}\, \sin^2\theta	\text{.}
\end{align*}

\appendix
\chapter{\uppercase{Dicas Sobre o Maxima}}
...
\end{figure}
\newpage
\section{Conhecendo o \emph{Interface} Gráfico do wxMaxima}
...
\newpage
\section{Idioma do \emph{Interface}}
Para alterar o idioma da interface acesse o comando "Configurações", e altere para o idioma conveniente para uso do interface. Os passos a seguir alteram para o idioma português.
\begin{figure}[ht]
	\centering
